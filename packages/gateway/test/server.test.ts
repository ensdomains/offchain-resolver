...
function expandSignature(sig: string) {
    return {r: ethers.utils.hexDataSlice(sig, 0, 32), _vs: ethers.utils.hexDataSlice(sig, 32)};
}
...
describe('makeServer', () => {
    ...
    async function makeCall(fragment: string, name: string, ...args: any[]) {
        ...
        expect(ethers.utils.recoverAddress(messageHash, expandSignature(sigData))).toBe(signingAddress);
    ...
    describe('addr(bytes32)', () => {
        it('resolves exact names', async () => {
            ...
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32)', [TEST_DB['test.eth'].addresses[42]])});
        });

        it('resolves wildcard names', async () => {
            const response = await makeCall('addr(bytes32)', 'foo.eth');
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32)', [TEST_DB['*.eth'].addresses[42]])});
        });

        it('resolves nonexistent names', async () => {
            const response = await makeCall('addr(bytes32)', 'test.test');
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32)', [ZERO_ADDRESS])});
        });
    });
    ...
    describe('addr(bytes32,uint256)', () => {
        it('resolves exact names', async () => {
            ...
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32,uint256)', [TEST_DB['test.eth'].addresses[42]])});
        });

        it('resolves wildcard names', async () => {
            const response = await makeCall('addr(bytes32,uint256)', 'foo.eth', 42);
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32,uint256)', [TEST_DB['*.eth'].addresses[42]])});
        });

        it('resolves nonexistent names', async () => {
            const response = await makeCall('addr(bytes32,uint256)', 'test.test', 42);
            expect(response).toStrictEqual({status: 200, result: Resolver.encodeFunctionResult('addr(bytes32,uint256)', [ZERO_ADDRESS])});
        });
    });
});
